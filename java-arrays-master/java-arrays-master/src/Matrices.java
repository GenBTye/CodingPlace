...
public class Matrices {
    ...
    public static class Vector {
        ...
        public Vector add(Vector other) {
            ...
            int[] sum = new int[this.items.length];
        ...
        public Vector subtract(Vector other) throws Exception {
            if (other.items.length != this.items.length) {
                throw new Exception("Different vector's lengths");
            }
            int[] sum = new int[this.items.length];
            for (int i = 0; i < this.items.length; i++) {
                sum[i] += this.items[i] - other.items[i];
        ...
        public int dotProduct(Vector other) throws Exception {
            ...
            int sum = 0;
            for (int i = 0; i < this.items.length; i++) {
                sum += this.items[i] * other.items[i];
            }
            return sum;
        }
    ...
    public static class Matrix {
            ...
            for (int i = 0; i < this.nRows; i++) {
                for (int k = 0; k < this.nCols; k++) {
                    for (int j = 0; j < this.nRows; j++) {
                        result.rows[i][k] += this.rows[i][j] * other.rows[j][k];
                    }
                }
            }
            return result;
        ...
        public int determinant() {
            ...
            for (int i = 0; i < this.nRows; i++) {
                ...
                Matrix smallerMatrix = new Matrix(this.nRows - 1, this.nRows - 1);
                int iRow = 1;
                int row = 0;
                while (iRow < this.nRows) {
                    ...
                    while (iCol < this.nCols) {
                        if (iCol != i) {
                            smallerMatrix.rows[row][col] = this.rows[iRow][iCol];
                            col++;
                            iCol++;
                        }
                        else iCol++;
                ...
                int subDeterminant = determinant(smallerMatrix.rows);
                ...
                result += sign * this.rows[0][i] * subDeterminant;
            }
            return result;
        }
        ...
        private static int determinant(int[][] matrix) {
            ...
            for (int i = 0; i < matrix.length; i++) {
                ...
                int iRow = 1;
                int row = 0;
                while (iRow < matrix.length) {
                    int col = 0;
                    int iCol = 0;
                    while (iCol < matrix[0].length) {
                        if (iCol != i) {
                            smallerMatrix[row][col] = matrix[iRow][iCol];
                            col++;
                            iCol++;
                        }
                        else iCol++;
